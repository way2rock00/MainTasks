...
export class SecureImagePipe implements PipeTransform, OnDestroy {
  ...
  transform(url: string): any {
    //Commented call to image service
    // console.log('Start Image:URL:'+url+' Date:'+new Date());    
    //  let obj = this.internalTransform(url);
    //  var objRet = this.asyncTrasnform(obj);
    //  console.log('End Image:URL:'+url+' Date:'+new Date());     
    //  return objRet;
    return "../../../assets/GoogleLogo.png";
  }

  private internalTransform(url: string): Observable<any> {
    // console.log('In internalTransform:'+url+':'+this.previousUrl)
    if (!url) {
      return this.result;
    }
