...
import { DeployTabDataService } from 'src/app/feature/deliver/services/deploy/deploy-tab-data.service';
...
export class DeployComponent implements OnInit {
  ...
  SUB_NAV: LAYOUT_DELIVER_SUB_NAV = LAYOUT_DELIVER_SUB_NAV.DEPLOY;
  ...
  constructor(private globalData: PassGlobalInfoService,
    ...
    private deployTabDataService: DeployTabDataService,
  ...
  ngOnInit() {
    ...
    this.deployTabDataService.clearData(); //Filter changes
  ...
  ngOnDestroy() {
    ...
    if (this.toggledEvent == "DEPLOY TOGGLED") {
      ...
      const dialogref = this.dialog.open(TabChangeSaveDialogueComponent, {
        data: {
          ...
          tabContents: this.deployTabDataService.deployContentsJson,
  ...
  emitTabCount() {
    ...
    this.sharedService.tabCountEvent.emit({ "tabName": this.tab.tabName, "tabCount": this.utilService.getL2TabCount('', this.deployTabDataService.deployContentsJson[0].tabContent, 'L2grp', 'L2enabledflag','L2doclink') });
  }

  setFilters() {
    this.deployTabDataService.deployContentsJson = [];
    this.deployTabDataService.getTabDataURL(this.tab.serviceURL).subscribe(data => {
      this.deployTabDataService.deployContentsJson = this.utilService.formTabContents(data, this.tab.tabName, this.tab.tabStorage);
      this.emitTabCount();
    });
  }
  ...
  toggle(j, parent, flag) {
    ...
    this.toggledEvent = "DEPLOY TOGGLED"; //Filter changes
