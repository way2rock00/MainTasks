...
import { LAYOUT_CONFIGURATION, LAYOUT_DELIVER_SUB_NAV, LAYOUT_TYPE } from 'src/app/shared/constants/layout-constants';
...
@Component({
  selector: 'app-construct-page',
  templateUrl: './construct-page.component.html',
  styleUrls: ['./construct-page.component.scss']
})
export class ConstructPageComponent implements OnInit, OnDestroy {
  LAYOUT: LAYOUT_TYPE = LAYOUT_TYPE.DELIVER;
  SUB_NAV: LAYOUT_DELIVER_SUB_NAV = LAYOUT_DELIVER_SUB_NAV.CONSTRUCT;
  layoutConfig : string = LAYOUT_CONFIGURATION[this.LAYOUT][this.SUB_NAV];
  ...
  ngOnInit() {
    ...
    this.constructTabDataService.clearData();
  ...
  ngOnDestroy() {
    ...
    this.constructTabDataService.clearData();
    this.constructTabDataService.clearFilters();
  }
