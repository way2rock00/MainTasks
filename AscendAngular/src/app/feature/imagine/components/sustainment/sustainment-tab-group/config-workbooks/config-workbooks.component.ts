...
@Component({
  selector: 'app-config-workbooks',
  templateUrl: './config-workbooks.component.html',
  styleUrls: ['./config-workbooks.component.scss']
})
export class ConfigWorkbooksComponent implements OnInit {
  ...
  ngOnDestroy() {
    ...
    if (this.toggledEvent == "CONFIGWORKBOOKS TOGGLED") {
      ...
      const dialogref = this.dialog.open(TabChangeSaveDialogueComponent, {
        data: {
          ...
          tabContents: this.sustainmentTabDataService.configWorkbooksContentsJson,
  ...
  emitTabCount() {
    ...
    this.sharedService.tabCountEvent.emit({ "tabName": this.tab.tabName, "tabCount": this.utilService.getL2TabCount('', this.sustainmentTabDataService.configWorkbooksContentsJson[0].tabContent, 'L2grp', 'L2enabledflag','L2doclink') });
  }

  setFilters() {
    this.sustainmentTabDataService.configWorkbooksContentsJson = [];
    this.sustainmentTabDataService.getTabDataURL(this.tab.serviceURL).subscribe(data => {
      this.sustainmentTabDataService.configWorkbooksContentsJson = this.utilService.formTabContents(data, this.tab.tabName, this.tab.tabStorage);
      this.emitTabCount();
    });
  ...
  toggle(j, parent, flag) {
    ...
    this.toggledEvent = "CONFIGWORKBOOKS TOGGLED"
