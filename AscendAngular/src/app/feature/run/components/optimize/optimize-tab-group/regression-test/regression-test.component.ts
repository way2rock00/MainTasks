...
@Component({
  ...
  styleUrls: ['./regression-test.component.scss']
})
export class RegressionTestComponent implements OnInit {
  ...
  constructor(private globalData: PassGlobalInfoService,
    ...
    ngOnDestroy() {
      ...
      if (this.toggledEvent == "REGRESSIONTEST TOGGLED") {
        ...
        const dialogref = this.dialog.open(TabChangeSaveDialogueComponent, {
          data: {
            ...
            tabContents: this.optimizeTabDataService.regressionTestContentsJson,
    ...
    emitTabCount() {
      ...
      this.sharedService.tabCountEvent.emit({ "tabName": this.tab.tabName, "tabCount": this.utilService.getL2TabCount('', this.optimizeTabDataService.regressionTestContentsJson[0].tabContent, 'L2grp', 'L2enabledflag','L2doclink') });
    }
    ...
    setFilters(){
      ...
      this.optimizeTabDataService.getTabDataURL(this.tab.serviceURL).subscribe(data => {
        this.optimizeTabDataService.regressionTestContentsJson = this.utilService.formTabContents(data, this.tab.tabName, this.tab.tabStorage); 
        this.emitTabCount();
      });
  ...
  toggle(j, parent, flag) {
    ...
    this.toggledEvent = "REGRESSIONTEST TOGGLED";
    ...
    this.emitTabCount();//Filter changes
    event.stopPropagation();
  }
